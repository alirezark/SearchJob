...
/**
 * Generates an array of sizes containing as many sizes as there are DEFAULT_SIZES_MULTIPLIERS, based on a "medium" logos sizes.
 * Each generated size contains a logos data model.
 * Allows for customized image, width, height per size,
 *
 * @param baseWidth {number} Value to use as base for a logos of 'medium' size
 * @param baseHeight {number} Value to use as base for a logos of 'medium' size
...
/**
 * Resolve the size to apply to a logos, based on whether its height or width was provided.
 * If both are provided, then do not changes them.
 * If none are provided, then use the logos default values.
...
export const resolveSize = ({
  ...
  if (width && height) {
    // Both width and height provided, which means we force the logos's size
    return {
      width: width,
      height: height,
    };
    ...
    // If no height nor width are provided, use the default logos values
    return {
      width: logo.width,
      height: logo.height,
    };
